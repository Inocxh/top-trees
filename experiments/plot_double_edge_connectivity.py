...
(sizes, values, yerr) = load_values(
	sys.argv[1], [
		"top_construction", "top_op",
		"topology_construction", "topology_op", 
        "splay_construction", "splay_op", 
	], skip_fields=4
)
...
plt.savefig("double_edge_connectivity_op.pdf", bbox_inches='tight')
plt.close()
...
plt.legend(['Self adjusting trees', 'Topology trees', 'Splay top trees'], loc='upper left')
plt.grid(True)

plt.savefig("double_edge_connectivity_construction.pdf", bbox_inches='tight')
plt.close()
